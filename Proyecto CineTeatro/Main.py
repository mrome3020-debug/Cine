...
from Validacion import administradores
...
def validar_admin_web(request):
	...
	if usuario in administradores and contrasena == administradores[usuario]['password']:
		request.session['usuario'] = administradores[usuario]['nombre']
	...
	return _render_login(request, error='Usuario o contraseña incorrectos.')
...
def registrar_cliente_web(request):
	...
	if not gmail or not usuario or not contrasena or not confirmacion:
		return _render_login(request, registro_error='Todos los campos de registro son obligatorios.')

	if not es_gmail_valido(gmail):
		return _render_login(request, registro_error='Debes ingresar un Gmail válido con formato usuario@gmail.com.')

	if len(contrasena) < 8:
		return _render_login(request, registro_error='La contraseña debe tener al menos 8 caracteres.')

	if contrasena != confirmacion:
		return _render_login(request, registro_error='La contraseña y su confirmación no coinciden.')

	ok, mensaje = registrar_cliente(gmail, usuario, contrasena)

	if not ok:
		return _render_login(request, registro_error=mensaje)
